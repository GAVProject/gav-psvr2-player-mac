...
- Without a headset display the player falls back to a preview window on the main screen (and without USB it renders with default calibration and no tracking; proximity-sensor logic is disabled there), so the build can be smoke-tested without hardware: `player/play some_180_SBS.mp4`, then check the log. Hot-plugging and USB reconnect are not supported — the player asks for a restart / shows a "tracking lost" notice.
...
- Camera thread (passthrough): interface 6, EP 0x87, `VI` packets carrying two 1024×1016 BC4 planes. It uses its own mutex so it never stalls tracking, the enable command must be sent *after* claiming the interface, and the stream is silenced before the device is closed (use-after-free on exit otherwise). `PassthroughSource` serializes camera start/stop on its own queue — shutdown blocks (thread join + control transfer) and must not run on the main/render thread; `stopAndWait()` precedes `psvr2_stop()`.
