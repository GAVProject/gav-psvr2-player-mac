...
видео по направлению взгляда, удобно лёжа) · `F` проекция · `G` стерео ·
...
- `psvr2_camera` — passthrough: команда включения камер `0x0B` уходит без
  ошибок, но кадров с сигнатурой `VI` не появляется ни на одном эндпоинте.
  **Сквозного видео получить не удалось**
